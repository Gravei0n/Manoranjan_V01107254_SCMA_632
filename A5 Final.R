...
# District mapping for Uttar Pradesh
district_mapping_up <- c(
  "1" = "Saharanpur",
  "2" = "Muzaffarnagar",
  "3" = "Bijnor",
  "4" = "Moradabad",
...
upnew$District <- ifelse(upnew$District %in% names(district_mapping_up), district_mapping_up[upnew$District], upnew$District)
upnew$Sector <- ifelse(upnew$Sector %in% names(sector_mapping), sector_mapping[upnew$Sector], upnew$Sector)

View(upnew)

hist(upnew$total_consumption, breaks = 10, col = 'blue', border = 'black', 
     xlab = "Consumption", ylab = "Frequency", main = "Consumption Distribution in UP State")
...
data_map <- st_read("E:\\VCU Bridge course\\UTTAR PRADESH_DISTRICTS.geojson") 
View(data_map)
